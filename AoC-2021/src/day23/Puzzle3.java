...
public class Puzzle3 extends PuzzleCommon
{
    ...
    public static Graph<Integer, String> initGraph(int group)
    {
        ...
        int[] grA = new int[] {8, 12, 13, 14};
        for (int i = 0; i < 4; i++)
        {
            int v = grA[i];
            result.addEdge(v, 1, groupA, 3+i);
            result.addEdge(v, 2, groupA, 2+i);
            result.addEdge(v, 3, groupA, 2+i);
            result.addEdge(v, 4, groupA, 4+i);
            result.addEdge(v, 5, groupA, 6+i);
            result.addEdge(v, 6, groupA, 8+i);
            result.addEdge(v, 7, groupA, 9+i);
        }

        int[] grB = new int[] {9, 15, 16, 17};
        for (int i = 0; i < 4; i++)
        {
            int v = grB[i];
            result.addEdge(v, 1, groupB, 5+i);
            result.addEdge(v, 2, groupB, 4+i);
            result.addEdge(v, 3, groupB, 2+i);
            result.addEdge(v, 4, groupB, 2+i);
            result.addEdge(v, 5, groupB, 4+i);
            result.addEdge(v, 6, groupB, 6+i);
            result.addEdge(v, 7, groupB, 7+i);
        }
        
        int[] grC = new int[] {10, 18, 19, 20};
        for (int i = 0; i < 4; i++)
        {
            int v = grC[i];
            result.addEdge(v, 1, groupC, 7+i);
            result.addEdge(v, 2, groupC, 6+i);
            result.addEdge(v, 3, groupC, 4+i);
            result.addEdge(v, 4, groupC, 2+i);
            result.addEdge(v, 5, groupC, 2+i);
            result.addEdge(v, 6, groupC, 4+i);
            result.addEdge(v, 7, groupC, 5+i);
        }
        
        int[] grD = new int[] {11, 21, 22, 23};
        for (int i = 0; i < 4; i++)
        {
            int v = grD[i];
            result.addEdge(v, 1, groupD, 9+i);
            result.addEdge(v, 2, groupD, 8+i);
            result.addEdge(v, 3, groupD, 6+i);
            result.addEdge(v, 4, groupD, 4+i);
            result.addEdge(v, 5, groupD, 2+i);
            result.addEdge(v, 6, groupD, 2+i);
            result.addEdge(v, 7, groupD, 3+i);
        }
